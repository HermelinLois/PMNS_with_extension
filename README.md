...
- **Note:** For very large primes, the automatically found `n_opt` might not be high enough, leading to an unsuccessful PMNS construction for that specific $n$. In such cases, we recommend building a suitable construction for the target prime and manually setting the parameters in the sparse generation workflow.
