...
- As a test workflow for conversions and reductions, we use Python to generate input data and expected results. In C, we use the input data and compare the computed result with the Python reference. Depending on the chosen PMNS parameters, test generation can be time-consuming when the number of tests is large. If these tests are not needed, we recommend reducing NTESTS or setting it to 0.
- If compilation fails with `gmp.h: No such file or directory`, ensure your environment (conda) is activated or pass `INCLUDE_DIR`/`LIBRARIES_DIR` to `make`.
